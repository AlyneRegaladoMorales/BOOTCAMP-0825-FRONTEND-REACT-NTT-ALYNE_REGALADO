...
export const getProductsByCategory = async (c: string): Promise<Product[] | undefined> => {
  try {
    const response = await fetch(`${API_URL}/products/category/${encodeURIComponent(c)}`);
    if (!response.ok) {
      console.error("Error en la peticion xxxxx", response.status);
      return;
    }
    const data = await response.json();
    const products = getAllProductsMapper(data.products);
    console.log("Porducto segun categoria", products)
    return products
  } catch (error) {
